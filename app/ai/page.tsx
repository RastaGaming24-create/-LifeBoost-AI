...
export default function AIPage() {
  ...
  async function askAI(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "No se pudo conectar con LifeBoost AI.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <AuthGuard>
      <main className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="mx-auto max-w-5xl px-6 py-12 lg:px-8">
          ...
          <p className="mt-3 max-w-2xl text-slate-400">Pregúntale a LifeBoost AI sobre presupuesto, ahorro, deudas y metas.</p>
          <div className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
            <form onSubmit={askAI}>
              <label className="text-sm text-slate-400">
                ¿Qué quieres analizar?
                <textarea required value={message} onChange={(e) => setMessage(e.target.value)} rows={5} maxLength={4000} className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none focus:border-blue-500" placeholder="Ej. ¿Cómo puedo ahorrar más dinero cada mes?" />
                <span className="mt-1 block text-right text-xs text-slate-600">{message.length}/4000</span>
              </label>
              <button disabled={loading} className="mt-4 rounded-xl bg-blue-600 px-5 py-3 font-semibold disabled:opacity-50">
                {loading ? "Analizando..." : "Preguntar a LifeBoost AI"}
              </button>
            </form>
          ...
          <p className="mt-4 text-xs text-slate-500">La clave de OpenAI se mantiene en el servidor y nunca se envía al navegador.</p>
