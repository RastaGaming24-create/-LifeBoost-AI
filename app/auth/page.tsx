...
function AuthPageContent() {
    ...
    try {
      ...
      if (mode === "signup") {
        ...
        if (!credential.user.emailVerified) {
          await sendEmailVerification(credential.user);
          await signOut(auth);
          throw new Error("Tu correo aún no está verificado. Te enviamos un nuevo enlace de verificación.");
        }
        router.replace(params.get("next") || "/dashboard");
      }
    } catch (err) {
  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      const profileName = credential.user.displayName?.trim() || existingData?.name || null;
      const profile = {
        email: credential.user.email,
        name: profileName,
        provider: "google",
      ...
      if (existing.exists()) {
        ...
        await setDoc(userRef, { ...profile, role: "user", createdAt: serverTimestamp() });
      }
      router.replace(params.get("next") || "/dashboard");
    } catch (err) {
      setError(firebaseAuthMessage(err));
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-6 py-16 text-white">
      <div className="mx-auto max-w-md">
        <a href="/" className="text-sm text-blue-400">← LifeBoost AI</a>
        <div className="mt-8 rounded-3xl border border-slate-800 bg-slate-900/80 p-7">
          <h1 className="text-3xl font-bold">{mode === "login" ? "Iniciar sesión" : "Crear cuenta"}</h1>
          <p className="mt-2 text-slate-400">Tu cuenta sincroniza tus datos de forma privada.</p>
          <button type="button" onClick={signInWithGoogle} disabled={disabled} className="mt-7 flex w-full items-center justify-center gap-3 rounded-xl border border-slate-700 bg-white px-5 py-3 font-semibold text-slate-900 disabled:opacity-50">
            <span className="text-lg font-bold">G</span>{googleBusy ? "Conectando con Google…" : "Continuar con Google"}
          </button>
          <div className="my-5 flex items-center gap-3 text-xs text-slate-500"><span className="h-px flex-1 bg-slate-800" /><span>o continúa con correo</span><span className="h-px flex-1 bg-slate-800" /></div>
          <form onSubmit={submit} className="space-y-4">
            ...
            <button disabled={disabled} className="w-full rounded-xl bg-blue-600 px-5 py-3 font-semibold disabled:opacity-50">{busy ? "Procesando…" : mode === "login" ? "Entrar" : "Crear cuenta"}</button>
          </form>
          {mode === "login" && <button onClick={resetPassword} disabled={disabled} className="mt-4 text-sm text-blue-400">¿Olvidaste tu contraseña?</button>}
          ...
          <button onClick={() => { setMode(mode === "login" ? "signup" : "login"); setError(""); setMessage(""); }} className="mt-6 text-sm text-slate-400">{mode === "login" ? "¿No tienes cuenta? Crear una" : "¿Ya tienes cuenta? Iniciar sesión"}</button>
        </div>
      </div>
    </main>
  );
...
export default function AuthPage() {
  return (
    <Suspense fallback={<main className="min-h-screen bg-slate-950 px-6 py-16 text-white"><div className="mx-auto max-w-md"><div className="mt-8 rounded-3xl border border-slate-800 bg-slate-900/80 p-7"><p className="text-slate-400">Cargando LifeBoost AI…</p></div></div></main>}>
      <AuthPageContent />
    </Suspense>
  );
}
