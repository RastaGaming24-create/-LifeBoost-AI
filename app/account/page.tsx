...
export default function AccountPage() {
  ...
  async function reportBug(e: React.FormEvent) {
    ...
    try {
      const token = await auth.currentUser?.getIdToken(true); if (!token) throw new Error("Sesión expirada.");
  ...
  async function checkout() {
    ...
    try { const token = await auth.currentUser?.getIdToken(true); const res = await fetch("/api/billing/checkout", { method: "POST", headers: { Authorization: `Bearer ${token}` } }); const data = await res.json(); if (!res.ok) throw new Error(data.error); if (data.url) window.location.href = data.url; }
    catch (e) { setStatus(e instanceof Error ? e.message : "No se pudo iniciar el pago."); setBusy(false); }
  }

  async function portal() {
    setBusy(true); setStatus("");
    try { const token = await auth.currentUser?.getIdToken(true); const res = await fetch("/api/billing/portal", { method: "POST", headers: { Authorization: `Bearer ${token}` } }); const data = await res.json(); if (!res.ok) throw new Error(data.error); if (data.url) window.location.href = data.url; }
  ...
  async function deleteAccount() {
    ...
    setBusy(true); setStatus("");
    try { const token = await auth.currentUser?.getIdToken(true); const res = await fetch("/api/account/delete", { method: "POST", headers: { Authorization: `Bearer ${token}` } }); const data = await res.json(); if (!res.ok) throw new Error(data.error); await auth.signOut(); window.location.href = "/"; }
  ...
  return <AuthGuard><main className="min-h-screen bg-slate-950 text-white"><Navbar/><div className="mx-auto max-w-5xl px-6 py-10 lg:px-8"><p className="text-sm font-medium text-blue-400">Cuenta</p><h1 className="mt-2 text-4xl font-bold">Mi cuenta</h1><p className="mt-2 text-slate-400">{user?.email}</p>
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6"><h2 className="text-xl font-semibold">LifeBoost Pro</h2><p className="mt-2 text-sm leading-6 text-slate-400">Desbloquea la experiencia Pro mediante una suscripción segura con Stripe.</p><button disabled={busy} onClick={checkout} className="mt-5 rounded-xl bg-blue-600 px-5 py-3 font-semibold disabled:opacity-50">Suscribirme a Pro</button><button disabled={busy} onClick={portal} className="ml-3 mt-5 rounded-xl border border-slate-700 px-5 py-3 font-semibold disabled:opacity-50">Gestionar facturación</button></section>
